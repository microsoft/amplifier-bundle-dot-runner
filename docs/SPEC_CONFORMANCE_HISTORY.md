# Spec Conformance Ledger — ARCHIVED

> **ARCHIVED 2026-09-02.** This file's *governing* role was retired when the repo adopted the
> Converge layout. It is retained, unedited below this banner, as the **dated decision record**
> for every `ATX-*` / `ULM-*` / `CAL-*` / `SYNC-*` / `DEAD-*` id — which `ledger/rows.yaml` rows
> cite by id (`decision.history`) and which `ledger/checks/` verifies still exists. Nothing here
> was deleted; the record is frozen, not rewritten.
>
> Its two live successors:
>
> | What moved | Where it lives now |
> |---|---|
> | The **Compatibility doctrine** (the five rules, maintainer ruling 2026-08-14) | [`docs/VISION.md`](VISION.md) |
> | The **`ATX-*` status table** (dispositions, asserted state) | [`ledger/rows.yaml`](../ledger/rows.yaml) |
>
> Everything else — the `ULM-*` and `CAL-*` gap tables, the DECIDE-item context, and the full
> dated Changelog — has no successor home and stays here. **Do not add new entries.** A new
> divergence is a `ledger/rows.yaml` row plus a `specs/EXTENSIONS.md` entry; a new doctrine
> ruling is a dated `docs/VISION.md` changelog entry.
>
> Paths inside the body below are as-written at archive time (`specs/canonical/…`,
> `specs/conformance/attractor-matrix.yaml`). They are historical citations, retained verbatim
> for ledger contiguity — the same convention `specs/EXTENSIONS.md` applies to its own entry
> bodies. Their current locations are `contracts/external/…` and `ledger/rows.yaml`.
